...
Fig.2b <- Data4 %>% ggplot()+
  ...
  scale_color_gradient2(low = "grey80", mid = "white", high = "#B24745", midpoint = 0) +
...
ggsave(str_c(pathout5,"/", "Ineq, CO2, and poverty outcome_SP_exp_for slides",".jpg"),
...
ggsave(str_c(pathout5,"/", "Ineq, CO2, and poverty outcome_SP_ImpProd_for slides",".jpg"),
       width = 10,height = 7.5,dpi = 500)
...
scenario_levels <- c("Explicit_Implicit_prod","Cash","SP_current","SP_covid")
...
Data7_min %>% ggplot( ) +
  ...
  scale_fill_manual(values = viridis_pal(option = "rocket")(7)[c(1,5:3)],
...
line_data <- Data7_min %>%
  filter(Scenario == "Explicit_Implicit_prod") %>%
