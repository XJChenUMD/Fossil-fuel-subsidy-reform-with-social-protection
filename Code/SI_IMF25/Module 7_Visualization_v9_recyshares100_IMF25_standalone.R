# Module 7 IMF25 STANDALONE (auto-adapted from Module 7 v9)
# Reference year: 2024 | Data: IMF 2025 | Paths: pathout2_imf25 / pathout5_imf25
# Independent copy -- edit freely for IMF25-specific tuning

#Module 7 (v9): Results summary and visualization
#Xiangjie Chen, GEOG, UMD
#xjchen@terpmail.umd.edu
# v9: aligned with Module 3 v4 (4 price scenarios), Module 4 v5 (5 SP, no Universal),
#     Module 6 v4 (100% recycling only from _v4/_v5 data)

library(ggpubr)
library(scales)


load(str_c(pathout2_imf25,"/Subsidy scenarios_price and CO2 response_v5.Rdata"))
...
#======================
load(str_c(pathout5_imf25,"/Results summary",Recynam[z],".Rdata"))

order.sub <- c("Explicit","Implicit_prod","Implicit_cons","Null")
...
Subsidy_2024 <- Subsidy_Clean %>% filter(year %in% 2024) %>% 
  ...
  filter(countryname %in% c(Reg_corr$IMF_reg,"Russian Federation")) %>% 
...
Subsidy_2024 %>% 
  filter(countrycode %in% targetISO) %>% 
  select(countryname,GDP,TotExpSub,TotImpSub,TotSub,Exp_gdpshare,Imp_gdpshare,Tot_gdpshare) -> Subsidy_2024_clean

Subsidy_2024_clean %>% bind_rows(setNames(Subsidy_agg,names(Subsidy_2024_clean))) %>% 
  mutate(countryname = factor(countryname, levels = rev(order.reg))) -> Subsidy_2024_clean

write.csv(Subsidy_2024_clean, str_c(pathout5_imf25,"/", "Subsidy to GDP_2024",".csv"))
...
ggsave(str_c(pathout5_imf25,"/", "FigS1_Fossil fuel subsidies as a share of GDP",".jpg"),
...
write.csv(Data1, str_c(pathout5_imf25,"/", "Fossil fuel subsidy by country_trend",".csv"))
...
Data1 %>%  mutate(era = if_else(year <= 2024, "2015-2024", "2025-2035")) %>% ggplot()+
  geom_bar(aes(year,TotalSub/1000, fill = Group, alpha = era),stat = "identity", position = "stack")+
  geom_vline(xintercept = 2024.5, linetype = "dotted", color =  "blue")+
  labs(x = NULL,y = "Fossil fuel subsidies (billion USD, 2017constant price)", fill = NULL)+
  facet_wrap(.~countryname,scales = "free_y",nrow = 4) +
  scale_alpha_manual(values = c("2015-2024" = 1, "2025-2035" = .75), guide = "none") +
  scale_fill_manual(values = viridis_pal(option = "rocket")(6)[c(2,3,4)],
...
ggsave(str_c(pathout5_imf25,"/", "Fossil fuel subsidy by country_trend_imp and exp",".jpg"),
...
Data1 %>% filter(Group %in% c("Implicit subsidy")) %>% 
  ...
  scale_fill_manual(values = wes_palette("Rushmore1", n = 5)[c(1,5,3)],
                    breaks = c("Explicit subsidy (Consumer)","Explicit subsidy (Producer)","Implicit subsidy"))+
  scheme
ggsave(str_c(pathout5_imf25,"/", "Fossil fuel subsidy by country_trend_imp",".jpg"),
       width = 10,height = 6.5,dpi = 500)
...
Data1 %>% filter(Group %in% c("Explicit subsidy (Consumer)","Explicit subsidy (Producer)")) %>% 
  ...
  scale_fill_manual(values = wes_palette("Rushmore1", n = 5)[c(1,5,3)],
                    breaks = c("Explicit subsidy (Consumer)","Explicit subsidy (Producer)","Implicit subsidy"))+
  scheme
ggsave(str_c(pathout5_imf25,"/", "Fossil fuel subsidy by country_trend_exp",".jpg"),
       width = 10,height = 6.5,dpi = 500)
...
Data1 %>% ggplot()+
  ...
  scheme
ggsave(str_c(pathout5_imf25,"/", "Fossil fuel subsidy by country_trend by sector_imp and exp",".jpg"),
       width = 10,height = 6.5,dpi = 500)
write.csv(Data1, str_c(pathout5_imf25,"/", "Fossil fuel subsidy by country_trend by sector_imp and exp",".csv"))
...
Data1 %>% ggplot()+
  ...
  scale_fill_manual(values = wes_palette("Rushmore1", n = 5)[c(1,5,4,3)])+
  scheme
ggsave(str_c(pathout5_imf25,"/", "Fossil fuel subsidy by country_trend by sector_exp",".jpg"),
       width = 10,height = 6.5,dpi = 500)
write.csv(Data1, str_c(pathout5_imf25,"/", "Fossil fuel subsidy by country_trend by sector_exp",".csv"))
...
Data1 %>% ggplot()+
  ...
  scale_fill_manual(values = wes_palette("Rushmore1", n = 5)[c(1,5,4,3)])+
  scheme
ggsave(str_c(pathout5_imf25,"/", "Fossil fuel subsidy by country_trend by sector_imp",".jpg"),
       width = 10,height = 6.5,dpi = 500)
write.csv(Data1, str_c(pathout5_imf25,"/", "Fossil fuel subsidy by country_trend by sector_imp",".csv"))
#===========================
...
write.csv(Data2,str_c(pathout5_imf25,"/", "Undesirable benefits distribution",".csv"))

country_order <- Data2 %>% 
  filter(SubScenario == "Explicit", ReGroup == "Top 10%") %>% 
  arrange(desc(Share)) %>% 
  pull(Country)
...
Data2 %>% 
  ...
  ggplot()+
  geom_bar(aes(Country,Share*100,fill = ReGroup),stat = "identity", position = "stack",width = 0.85, color = NA)+
  ...
  scale_fill_manual(values = viridis_pal(option = "rocket")(6)[4:6])+
  labs(fill = "Expenditure group", x = NULL, 
       y = "Proportion of benefits from fossil fuel subsidies")+
...
ggsave(str_c(pathout5_imf25,"/", "Undesirable benefits distribution_exp",".jpg"),
...
write.csv(Data3,
          str_c(pathout5_imf25,"/", "Incidence rate by decile_Total,Income and price effects",".csv"))

#Explicit subsidies
theme_2a <- theme_classic(base_size = 11) +
  theme(panel.grid.major.y = element_line(color = "grey99", linewidth = 0),
        panel.grid.minor = element_blank(),
        axis.line = element_line(linewidth = 0.3, colour = "black"),
        axis.ticks.length = unit(2, "pt"),
        axis.ticks = element_line(linewidth = 0.3, colour = "black"),
        axis.text.x = element_text(size = 8, angle = -270, vjust = .5,hjust=1),
...
Fig.2a <- Data3 %>% filter(SubScenario %in% "Explicit") %>%
  ...
  ggplot()+
  geom_bar(aes(Group,value*100,group = Vari,fill = Vari),stat = "identity", alpha=.7, size=0.8,width = 0.85)+
  ...
  labs(y = "Impacts on household expenditure",x = "Expenditure decile", fill = NULL)+
  theme_2a

Fig.2a_v2 <- Data3 %>% filter(SubScenario %in% "Explicit") %>%
  filter(!Vari %in% "Total_Effect") %>% 
  ggplot()+
  geom_bar(aes(Group,value*100,group = Vari,fill = Vari),stat = "identity", alpha=.7, size=0.8,width = 0.85)+
  facet_wrap(.~Country,ncol = 3,scales = "free_y") +
  scale_y_continuous(labels = function(x) paste0(x, "%"),
                     breaks = function(limits) { max_y <- limits[2]
                     round(c(0, max_y/2, max_y),2)})+
  scale_fill_manual(values = viridis_pal(option = "rocket")(6)[4:6],
                    labels = c("Income_Effect" = "Income Effect", 
                               "Price_Effect" = "Price Effect"))+
  scale_x_discrete(limits = DEC_name, breaks = c("D1", "D3", "D5", "D7", "D9"))+
  labs(y = "Impacts on household expenditure",x = "Expenditure decile", fill = NULL)+
  theme_2a+
  theme(strip.text = element_text(size = 5.5, face = "plain"),
        axis.text.x = element_text(size = 5.5),axis.text.y = element_text(size = 5.5),
        axis.title = element_text(size = 12,face = "bold"),
        axis.ticks = element_line(color = "black", linewidth = 0.05),
...
Fig.2b <- Data4 %>% ggplot()+
  ...
  geom_text_repel(aes(npl_chgrate,CO2_chgrate,label = Country),size = 3,force = 5,family = "sans",
  ...
  labs(y = "% change in carbon emissions", x = "% change in poverty headcount", 
       colour  = "Î” Gini\n coefficient")+
  guides(color = guide_colorbar(barwidth = 1, barheight = 5,frame.colour = "black", ticks.colour = "black"))+
  # scheme+
  theme_classic(base_size = 12) +
...
Fig.2c <- Data4 %>%
  ...
  coord_flip(clip = "off") +
  labs(x = NULL, y = "Î” poverty headcount (thousand)") +
...
Fig.2d <- Data4 %>%
  ...
  geom_text(aes(label = comma(round(CO2_chg,0))),
            hjust = -0.1, size = 3, family = "sans") +
  coord_flip(clip = "off") +
  labs(x = NULL, y = "Î” carbon emissions (MT)") +
  scale_y_continuous(expand = expansion(mult = c(0,0.1))) +
  theme_classic(base_size = 11)+
  theme(panel.grid.major.x = element_line(color = "grey85", linewidth = 0.3),
    panel.grid.minor = element_blank(),
...
write.csv(Data4, str_c(pathout5_imf25,"/", "Ineq, CO2, and poverty outcome_exp",".csv"))

ggarrange(ggarrange(Fig.2b, Fig.2d,Fig.2c, nrow = 1,widths = c(2,1,1),  
                    labels = c("(a)", "(b)", "(c)"), font.label = list(size = 12, face = "bold")), Fig.2a,
          heights = c(2,1.5),
          ncol = 1,labels = c(' ', "(d)"), font.label = list(size = 12, face = "bold"))

ggsave(str_c(pathout5_imf25,"/", "Fig2_Impact of subsidies removal_exp",".jpg"),
       width = 12,height = 10.5,dpi = 500)


ggarrange(ggarrange(Fig.2d,Fig.2c, nrow = 1,widths = c(1,1),  
                    labels = c("(a)", "(b)"), font.label = list(size = 12, face = "bold")))

ggsave(str_c(pathout5_imf25,"/", "Fig2bc_Impact of subsidies removal_exp",".jpg"),
       width = 6,height = 6,dpi = 500)


ggarrange(ggarrange(Fig.2b,Fig.2a_v2, nrow = 1,widths = c(1.8,1),  
                    labels = c("(a)", "(b)"), font.label = list(size = 12, face = "bold")))
ggsave(str_c(pathout5_imf25,"/", "Fig2ad_Impact of subsidies removal_exp",".jpg"),
       width = 6/1.8*2.8,height = 6,dpi = 500)
...
Fig.5c_impcons <-  Data4 %>% ggplot()+
  ...
  labs(y = "% change in carbon emissions", x = "% change in poverty headcount", 
       colour  = "Δ Gini coefficient")+
  guides(color = guide_colorbar(barwidth = 1, barheight = 5))+
  scheme

write.csv(Data4, str_c(pathout5_imf25,"/", "Ineq, CO2, and poverty outcome_ImpCons",".csv"))
...
write.csv(Data4, str_c(pathout5_imf25,"/", "Ineq, CO2, and poverty outcome_ImpProd",".csv"))
#===========================
...
Data6 %>% 
  # filter(!SPScenario %in% "PerfectTargeted") %>% 
  mutate(SPScenario = factor(SPScenario,levels = order.sp)) %>% 
  ggplot()+
  geom_point(aes(round(CO2_chgrate,0),round(npl_chgrate,0),fill = Gini_chg,shape = SPScenario),
  ...
  labs(x = "% change in carbon emissions", y = "% change in poverty headcount", 
  ...
  scale_fill_gradient2(name="Δ Gini coefficient", 
                       ...
                       midpoint = 0)+
  scale_shape_manual(values = c(21:26),breaks = order.sp,labels = order.spname)+
  ...
  theme(panel.grid.major = element_line(color = "grey90", linewidth = 0.3),
    ...
    panel.spacing    = unit(6, "pt"),
    plot.margin      = margin(5.5, 10, 5.5, 5.5))+
  theme(legend.position = "top",legend.box = "vertical",legend.background = element_blank(),
        legend.key.height = unit(8,"pt"),legend.key.width = unit(45,"pt"),
        legend.spacing.y = unit(10,"pt"),
        axis.title = element_text(face = "bold"))

ggsave(str_c(pathout5_imf25,"/", "Ineq, CO2, and poverty outcome_SP_exp",".jpg"),
       width = 9,height = 6,dpi = 500)
write.csv(Data6, str_c(pathout5_imf25,"/", "Ineq, CO2, and poverty outcome_SP_exp",".csv"))
...
ggsave(str_c(pathout5_imf25,"/", "Ineq, CO2, and poverty outcome_SP_ImpProd",".jpg"),
       width = 10,height = 7,dpi = 500)
write.csv(Data6, str_c(pathout5_imf25,"/", "Ineq, CO2, and poverty outcome_SP_ImpProd",".csv"))
...
ggsave(str_c(pathout5_imf25,"/", "Ineq, CO2, and poverty outcome_SP_ImpCons",".jpg"),
       width = 10,height = 7,dpi = 500)
write.csv(Data6, str_c(pathout5_imf25,"/", "Ineq, CO2, and poverty outcome_SP_Cons",".csv"))
#===========================
...
Lorenze$Benefit_cum[is.nan(Lorenze$Benefit_cum)] <- 0

Lorenze %>% filter(!Country %in% targetcounty) %>% 
  filter(Country %in% Reg_corr$WBGDPreg) %>% 
  left_join(Reg_corr %>% select(WBGDPreg,WB_IncomeGroup,Pop_2017_WB), by = c("Country" = "WBGDPreg")) %>% 
  filter(Scenario %in% c("Explicit","SP_covid","Cash","SP_current")) %>% 
...
Data7 %>% ggplot()+
  geom_abline(color = "grey75", linewidth = 0.5,linetype = 'dotted')+
  ...
  scale_color_manual(values = viridis_pal(option = "rocket")(7)[c(2:4,1)],
                    limits = c("Cash","SP_current","SP_covid","Explicit"),
  ...
  theme(panel.grid.major = element_line(color = "grey90", linewidth = 0.3),
    ...
    axis.ticks.length = unit(2, "pt"),
    axis.title = element_text(face = "bold"),
    axis.text.x = element_text(angle = -270, vjust = .5,hjust=1,size = 14),
    legend.position = "top",
    legend.text = element_text(size = 10),
...
ggsave(str_c(pathout5_imf25,"/", "Lorenze curve of benefit",".jpg"),
       width = 10,height = 7.5,dpi = 500)
write.csv(Data7, str_c(pathout5_imf25,"/", "Lorenze curve of benefit",".csv"))


Data7_min <- Data7 %>% mutate(BottomYS = abs(Pop_cum -0.5)) %>% 
  group_by(Country, Scenario) %>%
  slice_min(order_by = BottomYS, n = 1, with_ties = FALSE) %>%
  ungroup() 

scenario_levels <- c("Explicit","Cash","SP_current","SP_covid")
scenario_labels <- c("Fossil fuel subsidies","(S1) Cash-based\nprograms",
                     "(S2) Current social\nassistance",
                     "(S3) Social assistance\nCOVID-19 expansion")


Data7_min %>% ggplot( ) +
  geom_col(aes(x = Scenario, y = Benefit_cum*100, fill = Scenario), width = 0.75) +
  facet_wrap(~ Country, ncol = 6) +
  scale_fill_manual(values = viridis_pal(option = "rocket")(7)[c(1,5:2)],
                    limits = scenario_levels,
                    labels = scenario_labels,
                    guide = guide_legend(nrow = 1)) +
  scale_x_discrete(limits = scenario_levels, labels = NULL) +
  scale_y_continuous(labels = percent_format(scale = 1),
                     limits = c(0, 100), expand = expansion(mult = c(0, .05))) +
  labs(x = NULL, y = "Share of benefit for Bottom 50%", fill = NULL) +
  theme_classic(base_size = 12) +
  theme(panel.grid.major = element_line(color = "grey90", linewidth = 0.3),
        panel.grid.minor = element_blank(),
        axis.line  = element_line(color = "black", linewidth = 0.3),
...
ggsave(str_c(pathout5_imf25,"/", "Share of benefit_Bottom 50",".jpg"),
       width = 10,height = 7.5,dpi = 500)
write.csv(Data7_min, str_c(pathout5_imf25,"/", "Share of benefit_Bottom 50",".csv"))


line_data <- Data7_min %>%
  filter(Scenario == "Explicit") %>%
  select(Country, Benefit_cum) %>%
  mutate(Type = "Fossil fuel subsidies")

arrow_data <- Data7_min %>%
  filter(!Scenario %in% c("Explicit")) %>%
  group_by(Country) %>%
  slice_max(order_by = Benefit_cum, n = 1)

Data7_min %>%
  filter(Scenario != "Explicit") %>%
  ggplot() +
  geom_col(aes(x = Scenario, y = Benefit_cum * 100, fill = Scenario),
           width = 0.75) +
  geom_hline(data = line_data,
             aes(yintercept = Benefit_cum * 100, linetype = Type),
             color = "black", linewidth = .6) +
  geom_segment(data = arrow_data,
               aes(x = Scenario, xend = Scenario,
                   y = Benefit_cum * 100 + 5, yend = Benefit_cum * 100 + 1),
               arrow = arrow(length = unit(0.12, "inches"), type = "closed"),
               color = "black", linewidth = 0.4) +
  scale_x_discrete(limits = scenario_levels[2:4], labels = NULL) +
  facet_wrap(~ Country, ncol = 6) +
  scale_fill_manual(values = viridis_pal(option = "rocket")(7)[c(3,5,4)],
    limits = c("Cash", "SP_current", "SP_covid"),
    labels = c("(S1) Cash-based programs",
               "(S2) Current social assistance",
               "(S3) Social assistance COVID-19 expansion") ) +
  scale_linetype_manual(name = NULL,
    values = c("Fossil fuel subsidies" = "dashed"),
    guide = guide_legend(
...
ggsave(str_c(pathout5_imf25,"/", "Share of benefit_Bottom 50_v2",".jpg"),
       width = 9,height = 6,dpi = 500)
#===========================
...
SPCover_Decile %>% filter(!Reg %in% targetcounty) %>% 
  ...
  filter(!SPScenario %in% c("Null")) -> SPCover_Decile_agg

SPCover_Decile %>% filter(Reg %in% targetcounty) %>% 
  filter(!SPScenario %in% c("Null")) -> Data8
...
Data8  %>%  ggplot()+
  ...
  scale_color_manual(name = "", values = wes_palette("Royal1", n = 4)[c(2,1,4,3)],
                     breaks = order.sp,
                     labels = order.spname)+
...
ggsave(str_c(pathout5_imf25,"/", "Social protection coverage",".jpg"),
       width = 10,height = 7.5,dpi = 500)
write.csv(Data8, str_c(pathout5_imf25,"/", "Social protection coverage",".csv"))
#===========================
...
Price_data %>% group_by(Sec) %>% summarise(meanchange = mean(PriceChg)) %>% 
  arrange(desc(meanchange)) -> secorder

Price_data %>% filter(Sec %in% secorder$Sec[1:10]) %>% 
  ggplot() +
...
ggsave(str_c(pathout5_imf25,"/", "Price changes_exp",".jpg"),
       width = 10,height = 6,dpi = 500)
write.csv(Price_data, str_c(pathout5_imf25,"/", "Price changes_exp",".csv"))
#=============
...
annotate_figure(combined_plot,
                top = text_grob("Considering implicit subsidies as consumer subsidies\n", face = "bold", size = 14))

ggsave(str_c(pathout5_imf25,"/", "FigS4_Impact of subsidies removal_ImpCons",".jpg"),
...
annotate_figure(combined_plot,
                top = text_grob("Considering implicit subsidies as producer subsidies\n", face = "bold", size = 14))

ggsave(str_c(pathout5_imf25,"/", "FigS4_Impact of subsidies removal_ImpProd",".jpg"),
       width = 10,height = 10,dpi = 500)
