#Module 6 (v4): Results summary
#Xiangjie Chen, GEOG, UMD
#xjchen@terpmail.umd.edu
# v4: aligned with Module 3 v4 (4 price scenarios), Module 4 v6 (4 SP scenarios, PerfectTargeted removed),
#     Module 5 v6b (2 recycling shares: 80% and 100%)
...
for (z in 1:length(Recy)) {#loop for various recycling share
  ...
  a <- SP_Benifit_Bins[,,1,]  # Explicit scenario (index 1); Explicit_Implicit_prod removed in v4
  INT <- a
  ...
  SP_Benifit_Lorenz <- left_join(SP_Benifit %>% filter(!Scenario %in% "Null"),Pop_bins) %>% 
  ...
  for (i in 1:(dim(SP_Scenarios)[3])){
    # All slots 1-4 (Null, Cash, SP_current, SP_covid) use static SP_Scenarios array
  ...
  X_IPL <- OUT_SUBSP[,which(dimnames(OUT_SUBSP)[[2]] %in% "PR_npl_sub"),
                     which(dimnames(OUT_SUBSP)[[3]] %in% "Explicit"),
                     which(dimnames(OUT_SUBSP)[[4]]%in% c("Cash","SP_current","SP_covid"))]
  
  write.csv(X_IPL,str_c(pathout5,"/","National best policy mix_raw results_Explicit",Recynam[z],".csv"),row.names = F)
  ...
  #================
  #revenue-amount--------
  Revenue_nation <- Sub_revenue[,1:4]  # cols 1-4: Explicit, Implicit_prod, Implicit_cons, Null (col5=GDP)
  write.csv(Revenue_nation,
            str_c(pathout5,"/", "Revenue_nation.csv"))
  
  
  #revenue-share of GDP
  RevenuetoGDP <- Revenue_nation/Sub_revenue[,5]  # col5 = GDP in Module 3 v4
  #================
