...
Reg_corr_Full <- openxlsx::read.xlsx (str_c(pathdata4,
                                           "/Reg match table_GTAP_WBCD_WBGDP_IMF_GTAP11b160reg_Subsidy.xlsx"),
                                     sheet = 1)
Reg_corr <- Reg_corr_Full[which(Reg_corr_Full$Included %in% "Y"),]
Reg_corr <- Reg_corr[-which(substr(Reg_corr$GTAP.reg,1,1) %in% "X"),]
...
#Save results
#----------------
save(Saving_mat_detail,VA_mat_detail,Td_vec,Transfer_mat,TotOutput,
     VA_multiplier_Lab,VA_multiplier_Cap,VA_multiplier_Tax,FD,
     Reg_corr_Full,
     Footprint_detail,
